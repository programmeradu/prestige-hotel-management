...
$this->refs = 'https://prestahero.com/';
    ...
    private function getAccessories($list_id_products)
    {
        if(!$list_id_products)
            return false;
        return Db::getInstance()->executeS("SELECT * FROM "._DB_PREFIX_."product p, "._DB_PREFIX_."product_lang pl where p.id_product = pl.id_product ".(" AND p.id_product IN (".pSQL($list_id_products).")")."  AND pl.id_lang=".(int)Ets_mailchimpsync::getInstanceContext()->language->id.' GROUP BY p.id_product');
    }
    public function getContent()
	{
	    $ps_version = version_compare(_PS_VERSION_, '1.6.0', '>=') === true;
        if($ps_version)
        ...
        if($list_filters)
            ...
            {
                $filter['link_del'] = 'index.php?controller=AdminModules&configure=ets_mailchimpsync&tab_module=front_office_features&module_name=ets_mailchimpsync&token='.Tools::getAdminToken('AdminModules'.(int)(self::getIdFromClassName('AdminModules')).(int)$this->context->employee->id).'&delete_id='.$filter['id_export'];
                $list_id_products = explode('-',$filter['products']);
                $product_ids = [];
                foreach($list_id_products as $id_product)
                {
                    if(is_numeric($id_product) && $id_product > 0)
                        $product_ids[] = (int)$id_product;
                }
                if($product_ids)
                {
                    $products = Db::getInstance()->executeS("SELECT * FROM "._DB_PREFIX_."product p, "._DB_PREFIX_."product_lang pl where p.id_product = pl.id_product and p.id_product IN (".pSQL(implode(',', $product_ids)).")  AND pl.id_lang=".(int)Ets_mailchimpsync::getInstanceContext()->language->id.' GROUP BY p.id_product');
                    $list_name_product='';
                    if($products)
        ...
        $this->smarty->assign(
            array(                
                ...
                'id_export' => (int)Tools::getValue('id_export'),
                'delete_ok' => Tools::isSubmit('delete_ok'),
    ...
    public function getExportedList($id_export)
    {
        ...
        $sql = "SELECT c.id_customer,c.firstname,c.lastname,if(gl.name is not null, gl.name, '--') as gender,CONVERT(c.email, CHAR CHARACTER SET utf8) as email, if(c.birthday is not null AND c.birthday !='000-00-00', c.birthday, '--') as birthday, l.iso_code as lang,c.newsletter,c.optin, if(cl.name is not null, cl.name, '--') as country, if(sum(od.total_price_tax_incl), sum(od.total_price_tax_incl), 0) as total_price, if(cu.iso_code is not null,cu.iso_code,'--') as iso_code
                ...
                WHERE  
                    ...
                    (" AND c.id_customer IN (SELECT o.id_customer FROM "._DB_PREFIX_."orders o JOIN "._DB_PREFIX_."order_detail od ON o.id_order = od.id_order AND od.id_shop = ".(int)$this->context->shop->id." AND od.product_id IN(".pSQL($list_id_products).") GROUP BY o.id_customer)").
        ...
        if($newsletter && !$from_price && !$to_price && !$id_currency && !$id_category && !$id_country && ($optin==2 || $optin==0)  && !$id_lang && !$filter['products'])
        {
            ...
            elseif(Module::getModuleIdByName('blocknewsletter'))
