...
class BaVideoTab extends Module
{
    ...
    public function hookdisplayAdminProductsExtra($params)
    {
        ...
        if ($this->context->cookie->{'editfrommodule'} == 1) {
            $this->context->smarty->assign('editfrommodule', 1);
            $this->context->cookie->{'editfrommodule'} = 0;
        } else {
            $this->context->cookie->{'type'} = 0;
            $this->context->smarty->assign('editfrommodule', 0);
        }
        if (Tools::version_compare(_PS_VERSION_, '1.7', '>=')) {
    ...
    public function videoList()
    {
        ...
        //$helper->toolbar_btn = array('back' => true);
    ...
    public function deletea()
    {
        ...
        $id_product = (int) $data['0']['id_product'];
        $id_lang = (int) $data['0']['id_lang'];
        $type = $data['0']['type'];
        if ($type == 1) {
    ...
    public function delete()
    {
        ...
        foreach ($idArray as $key => $value) {
            $sql = 'SELECT * FROM ' . _DB_PREFIX_ . "url_video WHERE id_video='" . (int) $value . "' AND id_store='" . $id_shop . "'";
            $data = $db->ExecuteS($sql);
            $id_product = (int) $data['0']['id_product'];
            $id_lang = (int) $data['0']['id_lang'];
            $type = $data['0']['type'];
