...
function anadirMascara(){
    $('body').append('<div class="maskTPVTienda"></div>');
    $('.maskTPVTienda').show();
...
function salvarFirma(signaturePad){
    if (signaturePad.isEmpty()) {
        ...
        $('.maskTPVTienda').trigger("click");
        $.post(token_actionsFirma,{action:"anadirFirma", ajax:"1", id_order: id_order, img:signaturePad.toDataURL().substring(22, signaturePad.toDataURL().length),
            id_order_invoice:id_order_invoice,tipo:tipo},function(data) {
            if(data == 1){
                var msg = {message: 'firmado',type : 'firma'};
        		sendMessageToPeer(origen,JSON.stringify(msg));
        	}
        });
    }
}
function main(){
    ...
    var connectorInstance = new BNCConnector("pa");
    ...
    connectorInstance.listen = function(who,msg){
        ...
        var cliente = mensaje.cliente; //cliente text
        if(type == 'cerrarCosas')
        {
            $("#cambioEntregado").hide();
            $("#QR").hide();
        }
        if(type == 'qr')
        {
            $("#QR").show();
            $("#QR .textoCodebar").html(umsg['title']);
            $("#QR .totalQR").html(formatCurrency(parseFloat(umsg['totalQR']), currencyFormat, currencySign, currencyBlank));
            $("#QR .contCodebar .etiqueta").barcode(umsg['qr'], "datamatrix", settings);
        }
        if(type == 'totales')
        {
            if(mostrarPrecios == 1){
                $('#total').html(formatCurrency(parseFloat(umsg['total']), currencyFormat, currencySign, currencyBlank));
                $('#descuentos').html(formatCurrency(parseFloat(umsg['desc']), currencyFormat, currencySign, currencyBlank));
            }
            setTimeout(function(){$("#cambioEntregado").hide()}, 7000);
        }
        if(type == 'cambioEntregado'){
            $("#cambioEntregado").show();
            $('#cambio').html(formatCurrency(parseFloat(umsg['cambio']), currencyFormat, currencySign, currencyBlank));
            $('#entregado').html(formatCurrency(parseFloat(umsg['entregado']), currencyFormat, currencySign, currencyBlank));
        }
        if(type == 'compra')
        {
            $('.productos').html('');
            var html = '';
            var img_width_inicial = 0;
            $.each(umsg, function(key, value) {
                html+='<div class="row '+(img_width_inicial != 0 ? 'restoLista' : '')+'">'+(value['img'] != undefined ? '<img width="'+(img_width_inicial == 0 ? '130':'90')+'" src="'+value['img']+'"/>' : '<img width="130" src="../modules/tpvtienda/img/product.jpg"/>')+'<div class="contentProd"><h2>'+value['name']+'</h2>';
                html+='<div class="contInfoPantalla">'+value['qty']+'';
                if(mostrarPrecios==1)
                    html+=' x '+formatCurrency(parseFloat(value['price']), currencyFormat, currencySign, currencyBlank);
                html+="</div></div></div>";
                img_width_inicial++;
            });
            $('.productos').html(html);
            if(cliente != ""){
                $("#customerSeleccionado").html(cliente);
                $("#customerSeleccionado").show();
            }else
                $("#customerSeleccionado").hide();
        ...
        if(type == 'firma')
        {
            $.each(umsg, function(key, value) {
                origen = umsg.origen;
                id_order = umsg.id_order;
                id_order_invoice = umsg.id_order_invoice;
                tipo = umsg.tipo;
            });
...
//		console.log("Succesively sent to IP "+o[1]+" the message: "+o[0]);
    ...
    window.sendMessageToPeer = function (peerName,msg){
    	...
    	//document.ondblclick = function (){console.log(connectorInstance.getAllPeers());};
}
function resizePantalla(){
    var altura = $(window).height();
    var anchura = $(window).width();
//	var alturamenuBarPos = $('#menuBarPos').height();
    var alturamenuBarPos = 35;

    var alturaFooter = $('#footerTPV').height();
    if($("#pantallaTPV").hasClass("v15")){
        var alturaMultishop = $('.multishop_toolbar').height();
        var alturaPantalla = altura-(alturaCabecera+alturaMultishop+alturamenuBarPos+40)-6;
    }else{
        if(!$("#pantallaTPV").hasClass("expandido")){
            if($("#pantallaTPV").hasClass("bomenu1")){
                //menu vertical
                if($("#pantallaTPV").hasClass("v17") || $("#pantallaTPV").hasClass("v80") && anchura <= 1040)
                    var alturaCabecera = 60;
                if($("#pantallaTPV").hasClass("v17") || $("#pantallaTPV").hasClass("v80") && anchura > 1041)
                    var alturaCabecera = 40;  // 40+margen
                if($("#pantallaTPV").hasClass("v17") || $("#pantallaTPV").hasClass("v80"))
                    var alturaPantalla = altura-(alturaCabecera);
                if($("#pantallaTPV").hasClass("v16")) {
                    var alturaCabecera = 36;
                    var alturaPantalla = altura-(alturaCabecera);
                }
            }else{
                //menu horizontal
                var alturaMenu = $('#nav-topbar').height();
                var alturaPantalla = altura-(alturaMenu+alturamenuBarPos);
            }
        }else{
            var alturaPantalla = altura;
        }
        alturaPantalla -= $("#header").height();
    }
    $('.productos').css({"height":alturaPantalla-$('#footerPantalla').height()-1+'px'});
    $('#pantallaTPV').css({"min-height":alturaPantalla+'px'});
}
...
$(document).ready(function() {
   	main();
    setTimeout(function(){
        var msg = {message: 'init',type : 'init'};
        sendMessageToPeer('tp',JSON.stringify(msg));
        sendMessageToPeer('tp',JSON.stringify(msg));
        sendMessageToPeer('tp',JSON.stringify(msg));
        $(".loading").hide();
    }, 2000);
...
$(window).load(function() {
    ...
    $("#contFlecha").on("click", function(event){
        quitarCabeceras('toggle');
        $("#pantallaTPV").addClass("expandido");
        // $('#menuBarScreen').hide();
        resizePantalla();
    });
    resizePantalla();
 //	$(window).on('mousemove', function(e) {
//	    $('#menuBarScreen').slideDown();
//	    if($('#menuBarScreen').hasClass('expandido')){
//	        setTimeout(function(){ $('#menuBarScreen').slideUp()}, 2000);
//        }else{
//          	setTimeout(function(){ $('#menuBarScreen').slideDown()}, 2000);
//	    }
//	});
    $(document).on("keydown",function (e) {
        var tecla = e.keyCode;
        if(tecla == 118){ // F7
            $("#contFlecha").trigger("click");
        }
    });
