...
class Upgrader
{
    ...
    const DEFAULT_FILENAME = 'qloapps.zip';
    const BASE_DOMAIN = 'api.qloapps.com/';
    const BASE_URL = 'http://'.self::BASE_DOMAIN;

    // public $addons_api = self::BASE_URL.'xml/addons/';
    public $addons_api = self::BASE_DOMAIN;
     ...
     */
    public function downloadLast($dest, $filename = 'qloapps.zip')
    {
        if (empty($this->link)) {
            $this->checkPSVersion();
        }
            ...
            foreach ($feed->channel as $channel) {
                ...
                foreach ($channel as $branch) {
                    ...
                    if (
                        ...
                        $this->todolist = (array) $branch->todolist->todo;
    ...
    protected function browseXmlAndCompare($node, &$current_path = array(), $level = 1)
    {
        foreach ($node as $key => $child) {
            if (is_object($child) && $child->getName() == 'dir') {
                ...
                } elseif (!$this->compareChecksum($fullpath, (string) $child) && substr(str_replace(DIRECTORY_SEPARATOR, '-', $relative_path), 0, 22) != 'modules-qloautoupgrade') {
