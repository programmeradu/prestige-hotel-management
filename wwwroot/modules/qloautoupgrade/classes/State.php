...
class State
{
    ...
    /**
     * modules_to_disable is an array of array(id_addons => name_module).
     *
     * @var array
     */
    private $modules_to_disable = array();
    ...
    public function initDefault(Upgrader $upgrader, $prodRootDir, $version)
    {
        ...
        $new_native_modules = array();
        $new_installed_modules = array();
        ...
        if (is_object($nativeMods)) {
            ...
            foreach ($nativeMods as $nativeModsType) {
                if ('native' == $nativeModsType->attributes()->type) {
                    foreach($nativeModsType as $mod) {
                        $new_installed_modules[] = (string) $mod['name'];
                    }
                }
                foreach($nativeModsType as $mod) {
                    $new_native_modules[] = (string) $mod['name'];
                }
            }
        }
        $this->setNativeModules($new_native_modules);

        $old_native_modules = array();
        $old_installed_modules = array();
        ...
        if (is_object($nativeMods)) {
            ...
            foreach ($nativeMods as $nativeModsType) {
                if ('native' == $nativeModsType->attributes()->type) {
                    foreach($nativeModsType as $mod) {
                        $old_installed_modules[] = (string) $mod['name'];
                    }
                }
                foreach($nativeModsType as $mod) {
                    $old_native_modules[] = (string) $mod['name'];
                }
            }
        }
        $this->setModulesToDisable(array_diff($old_native_modules, $new_native_modules));
        $this->setModulesToInstall(array_diff($new_installed_modules, $old_installed_modules));
    ...
    {
        return $this->modules_to_disable;
    }
    ...
    public function setModulesToDisable($modules_to_disable)
    {
        $this->modules_to_disable = $modules_to_disable;
