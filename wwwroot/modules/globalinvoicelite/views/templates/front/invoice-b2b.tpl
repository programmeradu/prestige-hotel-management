		...
		<tr>                   		
			 ...
			 {else}
				<table style="width: 100%; vertical-align:center; border-collapse: separate; border-spacing: 0; margin-bottom:10px;padding:0; font-size: 8pt" cellpadding="5" cellspacing="0">
					...
					<tr>
	            		<td style="width: 100%; text-align:right; background-color:#FFF; color:{if Configuration::get('FSPA_color')}{Configuration::get('FSPA_color')|escape:'htmlall':'UTF-8'}{else}#000000{/if};">
						...
						<td style="text-align: left; width: {if Configuration::get('FSPA_details') == 1}35%;{else}56%{/if}; border-right: 2px solid #FFF; ">
							{if Configuration::get('FSPA_images') == 1}
								<table width="100%">
									<tr>
										<td width="15%">
											{if Product::getCover($order_detail.product_id)}
...
<table style="width: 100%; vertical-align:top; text-align:right; font-size:8pt; line-height:1pt; padding:0;" cellpadding="5" cellspacing="0">
	...
	<!-- END CART RULES -->
	<tr>
		<td style="width:87%; color:#404040; border-top: 1px solid #DDD; border-bottom: 1px solid #DDD;">{l s='Shipping Cost' mod='globalinvoicelite'} {l s='Tax Incl.' mod='globalinvoicelite'} - {l s='Carrier:' mod='globalinvoicelite'} {$order_invoice->getCarrier($order_invoice->id)->name|escape:'htmlall':'UTF-8'}</td>		
		<td style="width:13%; color:#000000; border-top: 1px solid #DDD; border-bottom: 1px solid #DDD;">{displayPrice currency=$order->id_currency price=$order_invoice->total_shipping_tax_incl}</td>
	</tr>
	{if $order_invoice->getRestPaid() > 0}
	<tr style="color:red;">
		<td style="width:87%; font-weight: bold; border-top: 1px solid #DDD; border-bottom: 1px solid #DDD;">{l s='Remaining Amount Due' mod='globalinvoicelite'}</td>
		<td style="width:13%; border-top: 1px solid #DDD; border-bottom: 1px solid #DDD;">{displayPrice currency=$order->id_currency price=$order_invoice->getRestPaid()}</td>	
	</tr>
	{/if}
</table>
<!-- / DESCUENTO Y ENVÍOS -->
